...
public class ClientToMt5Service extends BaseService{
    ...
    public void getParameterFromMq(String msg, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long tag){
        try {
            ...
            String reqParameter = mt5HandleService.send2Mt5Before(msg);
            // 手动下单前的处理
            boolean saveResult = mt5HandleService.saveReqParameter(reqParameter);
            if (reqParameter != null && saveResult == true) {
